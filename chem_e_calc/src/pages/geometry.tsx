...
const CalculatorContainer = () => {
  ...
  const calcVolume = (inputArray: InputType[]) => {
    ...
    const inputs = [
      {
        type: 'diameter',
        value: diameterObj?.displayValue.value,
        unit: diameterObj?.displayValue.unit,
      },
      {
        type: 'height',
        value: heightObj?.displayValue.value,
        unit: heightObj?.displayValue.unit,
      },
    ]
    ...
    const answerValue =
      Math.PI *
      (diameterObj.calculatedValue.value / 2) ** 2 *
      heightObj.calculatedValue.value
...
const InputField = ({ data, onChangeUnit, onChangeValue }: InputFieldProps) => {
  const { id, label, placeholder, type, selected, displayValue, unitType } =
    data
    ...
    <div className="form-control w-full">
      ...
      <label className="input-group">
        <input
          ...
          disabled={selected}
          onChange={e => onChangeValue(id, Number(e.target.value))}
        />
        <select
          className="select input-bordered bg-base-200 text-base-content"
          value={unit}
          onChange={e => onChangeUnit(id, e.target.value)}
        >
